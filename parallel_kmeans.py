...
def daskml_kmeans(X, n_clusters=5, chunk_rows=10000):
    X_dask = da.from_array(X, chunks=(chunk_rows, X.shape[1]))
    model = DaskKMeans(n_clusters=n_clusters, random_state=0)
    model.fit(X_dask)
    return model
...
def minibatch_kmeans_dask(X, n_clusters=5, batch_size=1000):
    ...
    results = [partial_fit_chunk(chunk) for chunk in chunks]
...
def evaluate_model(model, X, y_true, name, t_start):
    if 'dask_ml' in str(getattr(type(model), '__module__', '')):
        X_input = da.from_array(X, chunks=(10000, X.shape[1]))
...
if __name__ == "__main__":
    ...
    t = time.time()
    model_daskml = daskml_kmeans(X, n_clusters=5)
    evaluate_model(model_daskml, X, y, "DaskML KMeans", t)